...
class SeniorUser(db.Model):
    ...
    #※本番はテーブル連携するので#外すfamily_id = db.Column(db.Integer, ForeignKey('FamilyUser.family_id'), unique=True)
...
class FamilyUser(db.Model):
    ...
    #※本番はテーブル連携するので#外す   senior = relationship('SeniorUser', back_populates='family')
    #※本番はテーブル連携するので#外す messages = relationship('Message', back_populates='family')
...
class HealthInformation(db.Model):
    ...
    #※本番はテーブル連携するので#外す  senior = relationship('SeniorUser', back_populates='health_information')
...
class Message(db.Model):
    ...
    #※本番はテーブル連携するので#外す family = relationship('FamilyUser', back_populates='messages')
